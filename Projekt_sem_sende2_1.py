def coden(Antwort):
    ...
    Naricht = Antwort #list( Antwort.split())  #Geben Sie die Naricht ein. Trenne Buchstaben und Zeichen durch leerzeichen
    Alphabet = []
    x = 97
    while x < 123:
        Buchstaben = chr(x)
    ...
    while x < Naricht_länge:
        ...
        neue_Zahl = Zahl * 2
        #print (neue_Zahl," ",end="")
        output_sender = output_sender+neue Zahl
        x = x+1
        return output_sender
    result_sender = coden(input())
    print (result_sender)
